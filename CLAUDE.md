...
### Core Components
- **minigu/core**: Public API and main database engine
- **minigu/gql/parser**: GQL (Graph Query Language) parser with lexer and AST
- **minigu/storage**: Storage layer with both OLTP and OLAP support
- **minigu/gql/execution**: Query execution engine with operators and evaluators
- **minigu/gql/planner**: Query planner and optimizer with binder
- **minigu/catalog**: Schema and metadata management
- **minigu/context**: Database and session context management
- **minigu/transaction**: Transaction management system
...
### Parser Development
The parser is located in `minigu/gql/parser/` and implements a complete GQL parser with:
...
1. Update the grammar in `minigu/gql/parser/src/parser/`
2. Add corresponding AST nodes in `minigu/gql/parser/src/ast/`
3. Implement binding logic in `minigu/gql/planner/src/binder/`
4. Add execution support in `minigu/gql/execution/`
5. Update the logical planner in `minigu/gql/planner/src/logical_planner/`
...
cargo test -p minigu-storage --test integration_tests       # Run storage integration tests
cargo test -p gql-parser --test parser_test                 # Run parser snapshot tests
```
...
Current development is on `dev-catalog-txn` branch, implementing system-level transactions that integrate catalog and storage operations. This builds upon the existing OLTP transaction support to enable DDL operations within transactions.
