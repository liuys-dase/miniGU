...
pub struct GraphReadSession {
    ...
    global_txn: Arc<Mutex<Option<Transaction>>>,
}
...
impl GraphReadSession {
    pub fn new(
        ...
        let txn = container.begin_transaction(isolation)?;
        let mem_txn = Arc::clone(txn.graph.mem());
        Ok(Self {
            container,
            graph,
            txn: mem_txn,
            global_txn: Arc::new(Mutex::new(Some(txn))),
        })
    }
    ...
    pub fn commit(&self) -> Result<(), TxnError> {
        let mut txn = self
            .global_txn
            .lock()
            .expect("graph read transaction mutex should not be poisoned");
        if let Some(mut txn) = txn.take() {
            txn.commit().map(|_| ())
        } else {
            Ok(())
        }
...
impl GraphContainer {
    ...
    pub fn open_read_session(self: &Arc<Self>) -> Result<GraphReadSession, TxnError> {
